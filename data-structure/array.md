...
var result4 = arr4.splice(1, 3, "test", "vae");
console.log("arr4：" + JSON.stringify(arr4));
console.log("result4：" + JSON.stringify(result4));
```
打印结果：
...
result1 =[1,2,3,"a","b","c"]
result2 =["a","b","c",1,2,3,"test","vae"]
```
从打印结果中可以看到，原数组并没有被修改。

##### join()

join()：将数组**转换为字符串**，返回结果为**转换后的字符串**（**不会改变原来的数组**）。
...
sort()：对数组的元素进行从**小到大来排序**（会改变原来的数组）。
...
上方的打印结果中，会发现，使用 sort() 排序后，数字11竟然在数字2的前面。这是为啥呢？因为上面讲到了，sort()方法是按照Unicode编码进行排序的。
...
常见的遍历数组的方法包括：every()、filter()、forEach()、map()、some()。这几个方法都不会修改原数组。
...
map()对数组中每一项运行回调函数，返回该函数的结果，组成的新数组（返回的是加工之后的新数组）。map的应用场景主要有以下两种。
...
打印结果：
```js
arr1:[{"name":"Vader","age":"28"},{"name":"Yoda","age":"32"}]

arr2:["Vader","Yoda"]

arr3:[{"myName":"Vader","myAge":"28"},{"myName":"Yoda","myAge":"32"}]
```